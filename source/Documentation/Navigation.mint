  ...
  fun render {
    <>
      <Ui.Box title=<>"Navigation"</>>
        <p>
          "In other component libraries navigation is rendered directly " \
          "with elements, in Mint UI it is rendered from data which enables " \
          "adapting the view to the screen size."
        </p>

        <p>
          "For example the navigation in "
          <a href="/components/ui-header">"Ui.Header"</a>

          " component on desktop is displayed as a navigation list, but on " \
          "mobile since there is no screen estate it's displaid as an "

          <a href="/components/ui-action-sheet">"Ui.ActionSheet"</a>
          "."
        </p>

        <video src={@asset(../../assets/videos/navitem.mp4)} controls="true"/>
      </Ui.Box>
              ...
              @format {
                [
                  ...
                  Ui.NavItem.Item(
                    action:
                      (event : Html.Event) {
                        Ui.Notifications.notifyDefault(<>"Item clicked!"</>)
                      },
